...
const Api = {
  ...
  login: async (
    ...
    let json = await apiFetchPost("auth/login", {
      login,
      password,
      responseFacebook,
      responseGoogle,
  ...
  getTasks: async (filterTitle, filterType, filterDate) => {
    let json = await apiFetchGet("get_task/" + getUser().id, {
      filterTitle,
      filterType,
      filterDate,
    });
  ...
  changeSelectedTask: async (id) => {
    let json = await apiFetchPost(
      "change_selected_task/" + id + "/" + getUser().id,
      {}
    );
    return json;
  },
  ...
  sendRepeatDays: async (id, daysRepeat) => {
    let json = await apiFetchPost("send_daysRepeat/" + getUser().id, {
      id,
      daysRepeat,
    });
    return json;
  ...
  changeSelectedObjective: async (id) => {
    let json = await apiFetchPost(
      "change_selected_objective/" + id + "/" + getUser().id,
      {}
    );
    return json;
  },
